...
describe('cypher query builder', () => {
  describe('with custom cypher', () => {
    ...
    test('works with params', () => {
      ...
      expect(buildCypher({ fieldName, query, isWrite: false }))
        .toMatchInlineSnapshot(`
        "WITH apoc.cypher.runFirstColumnSingle(\\"WITH $parent AS parent MATCH (user:User{id: $args.id}) RETURN user\\", {args: $testUser_args, parent: $parent}) AS \`testUser\`
        RETURN \`testUser\` {.id, .name} AS \`testUser\`"
      `);
    });
    ...
    test('works with nested params', () => {
      ...
      expect(buildCypher({ fieldName, query, isWrite: false }))
        .toMatchInlineSnapshot(`
        "WITH apoc.cypher.runFirstColumnSingle(\\"WITH $parent AS parent MATCH (user:User) RETURN user\\", {parent: $parent}) AS \`testUser\`
        RETURN \`testUser\` {.id, posts: [testUser_posts IN apoc.cypher.runFirstColumnMany(\\"WITH {parent} AS parent MATCH (parent)-[:AUTHOR_OF]->(post:Post) RETURN post LIMIT $args.limit\\", {args: $testUser_posts_args, parent: testUser}) | testUser_posts {.id, .title}]} AS \`testUser\`"
      `);
    });
