...
describe('cypher query builder', () => {
  describe('with builder queries', () => {
    ...
    test('works for a single query', () => {
      ...
      expect(buildCypher({ fieldName, query, isWrite: false }))
        .toMatchInlineSnapshot(`
                                        ...
                                        MATCH (testUser:User {id:$testUser.args.id})
                                        RETURN testUser {.id, .name}"
                              `);
    });
    ...
    test('works for a basic nested query', () => {
      ...
      expect(buildCypher({ fieldName, query, isWrite: false }))
        ...
        "WITH $parent AS parent
        MATCH (testUser:User {id:$testUser.args.id})
        RETURN testUser {.id, .name, posts: [(testUser)-[:HAS_POST]->(testUser_posts:Post) | testUser_posts {.id, .title}]}"
      `);
    });
    ...
    test('works for a deeply nested query (node queries)', () => {
      ...
      expect(buildCypher({ fieldName, query, isWrite: false }))
        ...
        "WITH $parent AS parent
        MATCH (testUser:User {id:$testUser.args.id})
        RETURN testUser {.id, .name, posts: [(testUser)-[:HAS_POST]->(testUser_posts:Post) | testUser_posts {.id, .title, tags: [(testUser_posts)-[:HAS_TAG]->(testUser_posts_tags:Tag) | testUser_posts_tags {.id, .name}], author: head([(testUser_posts)<-[:HAS_POST]-(testUser_posts_author:User) | testUser_posts_author {.id, .name}])}]}"
      `);
    });
    ...
    test('works for an edge query', () => {
      ...
      expect(buildCypher({ fieldName, query, isWrite: false }))
        ...
        "WITH $parent AS parent
        MATCH (testUser:User {id:$testUser.args.id})
        RETURN testUser {.id, .name, posts: [(testUser)-[testUser_posts:HAS_POST]->(:Post)| testUser_posts {.isAuthor, post: head([()-[testUser_posts:HAS_POST]->(testUser_posts_post:Post) | testUser_posts_post {.id, .title}])}]}"
      `);
    });
    ...
    test('works with write clauses', () => {
      ...
      expect(buildCypher({ fieldName, query, isWrite: false }))
        .toMatchInlineSnapshot(`
                                ...
                                MERGE (globalGroup)-[:HAS_USER]->(testUser)
                                SET testUser.name = $testUser.args.input.name
                                SET testUser.age = $testUser.args.input.age
                                DELETE deleteMe
  ...
  describe('with custom cypher', () => {
    ...
    test('works with params', () => {
      ...
      expect(buildCypher({ fieldName, query, isWrite: false }))
        .toMatchInlineSnapshot(`
                                        "WITH apoc.cypher.runFirstColumnSingle(\\"WITH $parent as parent MATCH (user:User{id: $args.id}) RETURN user\\", {args: $testUser.args, parent: $parent}) AS \`testUser\`
                                        RETURN \`testUser\` {.id, .name} AS \`testUser\`"
                              `);
    });
    ...
    test('works with nested params', () => {
      ...
      expect(buildCypher({ fieldName, query, isWrite: false }))
        .toMatchInlineSnapshot(`
                                        "WITH apoc.cypher.runFirstColumnSingle(\\"WITH $parent as parent MATCH (user:User) RETURN user\\", {parent: $parent}) AS \`testUser\`
                                        RETURN \`testUser\` {.id, posts: [testUser_posts IN apoc.cypher.runFirstColumnMany(\\"WITH $parent as parent MATCH (parent)-[:AUTHOR_OF]->(post:Post) RETURN post LIMIT $args.limit\\", {args: $testUser_posts.args, parent: testUser}) | testUser_posts {.id, .title}]} AS \`testUser\`"
                              `);
    });
    ...
    test('works with a write query', () => {
      ...
      expect(buildCypher({ fieldName, query, isWrite: true }))
        .toMatchInlineSnapshot(`
                                        "CALL apoc.cypher.doIt(\\"WITH $parent as parent CREATE (user:User{name: $args.name}) RETURN user\\", {args: $testCreateUser.args, parent: $parent})
  ...
  describe('with a combination of builder and custom', () => {
    test('works starting from custom cypher', () => {
      ...
      expect(buildCypher({ query, fieldName, isWrite: false }))
        .toMatchInlineSnapshot(`
        "WITH apoc.cypher.runFirstColumnSingle(\\"WITH $parent as parent MATCH (user:User {id: $args.id}) RETURN user\\", {args: $testUser.args, parent: $parent}) AS \`testUser\`
        RETURN \`testUser\` {.id, posts: [(testUser)-[:HAS_POST]->(testUser_posts:Post) | testUser_posts {.id, .title, tags: [testUser_posts_tags IN apoc.cypher.runFirstColumnMany(\\"WITH $parent as parent MATCH (parent)-[:HAS_TAG]->(tag:Tag)\\", {parent: testUser_posts}) | testUser_posts_tags {.id, .name}]}]} AS \`testUser\`"
      `);
    });
    ...
    test('works starting from builder', () => {
      ...
      expect(buildCypher({ query, fieldName, isWrite: false }))
        .toMatchInlineSnapshot(`
        "WITH $parent AS parent
        MATCH (user:User {id: $testUser.args.id})
        RETURN testUser {.id, posts: head([testUser_posts IN apoc.cypher.runFirstColumnSingle(\\"WITH $parent as parent MATCH (parent)-[:HAS_POST]->(post:Post)\\", {parent: testUser}) | testUser_posts {.id, tags: [(testUser_posts)-[:HAS_TAG]->(testUser_posts_tags:Tag) | testUser_posts_tags {.id, .name}]}])}"
      `);
    });
  });
});
